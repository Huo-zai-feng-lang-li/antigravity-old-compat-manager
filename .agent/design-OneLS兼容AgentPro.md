...
## 架构
- `runtime/OneLSAgentProxyBridge.cjs`：纯 Node 模块，扫描 `~/.antigravity/extensions/dao-agi.dao-proxy-pro-*`，排除精确 `.obsolete` 版本，选最高语义版本；从 `~/.dao/origin-port.json` 和用户名 FNV 端口发现候选代理；验证 `/origin/ping` 的 `self_file` 必须指向选中的安装目录。
