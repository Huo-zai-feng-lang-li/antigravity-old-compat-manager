...
> ⚠️ **历史标注（v9.9.528 起）**：本文档前半部分是 v9.9.524 之前的历史复盘，保留供参考。**当前新模型兼容流程已大幅简化**，详见文末「当前新模型兼容流程（v9.9.524+）」。旧的“显式映射 / main.js allowlist / Agent Pro 精确映射表”流程已废弃。
...
## 当前新模型兼容流程（v9.9.524+，已替代上文旧流程）

官方发布新模型（3.9 / 4.0 / 4.1 等）时，**绝大多数情况不需要改任何代码**：

1. **模型显示**：账号登录后官方本身返回全量模型列表（含新模型），插件模型解锁 v9.9.528 起默认禁用、纯透传。新模型自动出现在 IDE 模型选择器。
2. **模型调用**：插件的模型改写/动态映射从请求 URL /v1beta/models/{model}:generateContent 提取用户实际选择的模型名，自动改写 LS 占位符 gemini-2.5-pro。**任何新模型都自动适配，不需要改映射代码**。
3. **仅当需要过滤/推荐/隐藏某个新模型时**，才修改本项目（old-compat-manager）的 Workbench 白名单（scripts/StableMode.Core.psm1），然后重新「应用并启动」。
4. **验证**：在 IDE 选择新模型发一条消息，确认正常响应即可。

> 核心变化：旧流程需要“显式三元组映射 + main.js allowlist + Agent Pro 精确映射表”，现已被“插件动态映射 + 官方全量模型透传”取代。本项目只在需要过滤模型时才介入。
